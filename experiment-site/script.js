...
async function loadData() {
    try {
        ...
        const response = await fetch('/api/savedata'); // 向后端 API 发送 GET 请求
        if (!response.ok) {
            throw new Error('加载数据失败');
        }
        const data = await response.json(); // 获取后端返回的数据
        console.log('数据加载成功:', data);
        updateUIWithData(data); // 调用辅助函数更新页面
    } catch (error) {
    ...
    for (let week = 1; week <= 11; week++) {
        ...
        tabPanel.innerHTML = `
            ...
            <div class="file-upload">
                ...
                ${uploadedFile ? `<div class="uploaded-file-link"><a href="${uploadedFile}" target="_blank">查看文件</a></div>` : ''}
            </div>
        `;
